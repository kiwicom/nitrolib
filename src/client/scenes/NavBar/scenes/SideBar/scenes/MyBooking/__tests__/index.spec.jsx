...
describe("#MyBooking", () => {
  ...
  test("not handle non-existent field", () => {
    ...
    wrapper.instance().handleChange({ target: { id: "asdf", value: "omfg" } });
  ...
  test("handle change bid", () => {
    ...
    wrapper.instance().handleChange({ target: { id: "bid", value: "1234" } });
  ...
  test("handle normalize bid", () => {
    ...
    wrapper.instance().handleChange({ target: { id: "bid", value: "1234a" } });
  ...
  test("handle change email", () => {
    ...
    wrapper.instance().handleChange({ target: { id: "email", value: "lol@kek.bur" } });

    expect(wrapper.state("fields").email.value).toBe("lol@kek.bur");
  });

  test("handle validate email", () => {
    const wrapper = shallow(<MyBooking lang="en" />);

    wrapper.instance().handleChange({ target: { id: "email", value: "lol" } });

    expect(wrapper.state("fields").email.value).toBe("lol");
    expect(wrapper.state("fields").email.error).toBe("forms.wrong_format_email");
  });
